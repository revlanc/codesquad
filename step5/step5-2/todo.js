...
const ErrorHandler = function (controller, errorFontColor) {
    this.controller = controller;
    this.redColor = errorFontColor;
}
ErrorHandler.prototype = {
    getErrorType(errorMsg) {
        if (errorMsg.length === 4) errorMsg = 'SameStatusError' //errorMsg가 id이면(4글자) SameStatusError
        const ErrorType = {
...
const view = new View('\x1b[36m%s\x1b[0m');
const controller = new Controller(model, view);
const errorHandler = new ErrorHandler(controller, '\x1b[31m%s\x1b[0m');
const app = {
    util: util,
